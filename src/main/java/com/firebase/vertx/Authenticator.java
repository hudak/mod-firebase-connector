...
public class Authenticator extends AsyncTask<AuthData> {
  ...
  @Override protected void execute( final Promise<AuthData> future ) {
    ...
    if ( config instanceof JsonObject ) {
      ...
      if ( authMap.containsField( "email" ) && authMap.containsField( "password" ) ) {
        ...
        future.reject( new FirebaseException( "Unable to authenticate with " + authMap ) );
      }
      ...
      future.reject( new FirebaseException( "Unable to authenticate with " + config ) );
    }
  }
}
